...
const StoryTheaterSession: React.FC<Props> = ({ entry, preset, masks, onBack, onEdit, onOpenVectorMemory, onEntryChange }) => {
    ...
    return <div className='relative h-full w-full flex flex-col bg-stone-100 text-slate-800'>
        ...
        <footer className='story-safe-footer shrink-0 px-4 pt-3 bg-stone-100/95 backdrop-blur border-t border-slate-200'>
            <div className='max-w-2xl mx-auto'>
                ...
                <div className='flex items-end gap-2 p-2 rounded-2xl bg-white border border-slate-200 shadow-sm'>
                    <MobileAutoGrowTextarea minHeight={48} maxHeight={144} value={input} onChange={event => setInput(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && (event.ctrlKey || event.metaKey) && !event.nativeEvent.isComposing) { event.preventDefault(); void send(); } }} disabled={sending} placeholder={pendingRetryInput ? '留空并点击推进，可继续上次中断' : canWriteOpening ? '也可以先写一句；留空推进则由故事开场' : '写下动作、对白、时间跳转，或你希望故事发生的事……'} className='flex-1 px-2 py-2 leading-6 disabled:opacity-50' />
        ...
        {editingMessage && <VisualViewportFixedLayer zIndex={75} className='flex items-end overflow-y-auto overscroll-contain bg-slate-900/30' onClick={() => !mutatingMessage && setEditingMessage(null)}>
            {/* MOBILE_KEYBOARD_ANCHORING_HOTFIX：承载层锚 visual viewport bottom，键盘弹出时编辑框不再落进键盘后面。 */}
            <div data-keyboard-scroll className='story-safe-sheet story-keyboard-sheet flex max-h-full w-full flex-col overflow-y-auto overscroll-contain rounded-t-3xl bg-stone-100 px-5 pt-5 shadow-2xl' onClick={event => event.stopPropagation()}>
                ...
                <MobileAutoGrowTextarea ensureVisibleOnFocus={false} minHeight={192} maxHeight={384} autoFocus value={editDraft} onChange={event => setEditDraft(event.target.value)} className='mt-4 rounded-2xl border border-slate-200 bg-white p-4 leading-6' />
