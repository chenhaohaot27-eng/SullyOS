...
describe('forFirePack —— 打包时刻的状态一律不烤进模板', () => {
    ...
    it('「你刚刚和对方结束了一通电话 / 见面」不进', async () => {
        ...
        const modeTransitionMarker = '系统提示｜模式切换（最高优先级）';
        expect(await withMsgs(false)).toContain(modeTransitionMarker);
        expect(await withMsgs(true)).not.toContain(modeTransitionMarker);
    });
